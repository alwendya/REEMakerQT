...
PDGHelper::PDGHelper()
{
    // connect(this, &PDGHelper::EnvoiConsigne, this,
...
*/
void
PDGHelper::SetBaseModelePath(QString lBaseModelePath)
{
...
*/
bool
PDGHelper::OpenAndParseConfig_v2(QString CheminConfig)
{
    DocumentOuvert = "";
    ...
    while (!in.atEnd()) {
        ...
        if (line.length() > 2) {
            if (line.mid(0, 2) == "ï»") // C'est le BOM UTF8
                line = line.mid(3);     // Suppression BOM
            if (line.length() < 2)
                continue;                                         // Trop petit maintenant, n passe à l'item suivant
            if (line.mid(0, 2) == "::" || line.mid(0, 2) == "ï»") // C'est un commentaire on ignore ou le
                                                                  // BOM du fichier UTF8
                continue;
            line = line.trimmed();
    ...
    for (qsizetype compteLigne = 0; compteLigne < vecFichierPDG.size(); compteLigne++) {
        ...
        if (trimmedLine.mid(0, 2) != "--") // C'est pas un argument, on push le vec precedent puis on
                                           // re-on init mVecCommande avec la fonction
        {
            ...
            CmdKeys mCmdKeys;
            if (trimmedLine.contains('=')) { // Il y a un = c'et une variable
                ...
                mCmdKeys.Valeur = "true"; // On le met à true, l'important c'est
                                          // qu'il ait une valeur
            }
            mStructCommand.mVecCommande.push_back(mCmdKeys); // On crée le nouveau
        }
    }
    if (mStructCommand.mVecCommande.size() > 0)    // On a fini et un argument est construit
    ...
    for (qsizetype i = 0; i < vecCommandeList.size(); i++) {
        if (vecCommandeList[i].mTypeCommande == TypeCommande::DESSINECHECKBOXQUESTION ||
            ...
            if (vecCommandeList[i].mTypeCommande == TypeCommande::DESSINETEXTEQUESTION ||
                ...
                // Variable pour les questions texte ligne et multiligne
                mQuestion.NomVariable = RetourneCleStr(vecCommandeList[i].mVecCommande, "nomvariable", "");
            }
            if (vecCommandeList[i].mTypeCommande == TypeCommande::DESSINETEXTEQUESTION)
...
*/
bool
PDGHelper::BurstVersDisque(QString FichierSortie)
{
    if (vecCommandeList.size() == 0) {
        EnvoiLogVersMainWindows("INFO: Aucune commande à dessiner...");
        return false;
    }
    ...
    for (qsizetype lArg = 0; lArg < vecCommandeList.size(); lArg++) {
        ...
        for (qsizetype i = 0; i < vecCommandeList[lArg].mVecCommande.size(); i++) {
            ...
            // Cles sans valeur ==> FLAG
                ...
                f_out << "--" << wKeys << Qt::endl;
            // Cles avec string
            else if (wKeys == "texte" || wKeys == "question" || wKeys == "aidequestion" || wKeys == "chemin" || wKeys == "nomvariable")
                f_out << "--" << wKeys << "=" << wValeur << Qt::endl;
...
PDGHelper::DrawOnPage_v2(PoDoFo::PdfPainter& Painter, PoDoFo::PdfDocument& Document)
{
    ...
    /// Originals : CreateFontSubset(trueTypeFontName, fBold, fItalic, false,
    ...
    PoDoFo::PdfFont& pFontMono = Document.GetFonts().GetOrCreateFont(CheminRobotoMonoRegular.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontMonoBOLD = Document.GetFonts().GetOrCreateFont(CheminRobotoMonoBold.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontMonoITALIC = Document.GetFonts().GetOrCreateFont(CheminRobotoMonoItalic.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontMonoBOLDITALIC = Document.GetFonts().GetOrCreateFont(CheminRobotoMonoBoldItalic.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontReg = Document.GetFonts().GetOrCreateFont(CheminRobotoRegular.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontRegBOLD = Document.GetFonts().GetOrCreateFont(CheminRobotoBold.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontRegITALIC = Document.GetFonts().GetOrCreateFont(CheminRobotoItalic.toStdString().c_str(), paramsNoSubset);
    PoDoFo::PdfFont& pFontRegBOLDITALIC = Document.GetFonts().GetOrCreateFont(CheminRobotoBoldItalic.toStdString().c_str(), paramsNoSubset);

    if (pFontMono.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Monospace Regular"));
        return 0;
    }
    if (pFontMonoBOLD.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Monospace Gras"));
        return 0;
    }
    if (pFontMonoITALIC.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Monospace Italic"));
        return 0;
    }
    if (pFontMonoBOLDITALIC.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Monospace Gras / Italic"));
        return 0;
    }
    if (pFontReg.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Regular"));
        return 0;
    }
    if (pFontRegBOLD.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Gras"));
        return 0;
    }
    if (pFontRegITALIC.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Italic"));
        return 0;
    }
    if (pFontRegBOLDITALIC.GetName() == "") {
        EnvoiLogVersMainWindows(QString("PoDoFo n'a pu charger la police Roboto Gras / Italic"));
        return 0;
    }

    double PageWidth = Painter.GetCanvas()->GetRectRaw().GetWidth();
    Q_UNUSED(PageWidth);
    double PageHeight = Painter.GetCanvas()->GetRectRaw().GetHeight();
    /* -- Pré chargement des variables de la page de garde -- */
    ...
    for (qsizetype lArg = 0; lArg < vecCommandeList.size(); lArg++) {
        ...
        if (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTE ||
            ...
            valTexte = QString(valTexte).replace("\n", " "); // On supprime les multilignes si présentes
        foreach (auto var, vecVARIABLE) {
            valTexte.replace("%%" + var.Variable.toLower() + "%%", var.Valeur);
        }

        QString valQuestion = RetourneCleStr(vecCommandeList[lArg].mVecCommande, "question");
        Q_UNUSED(valQuestion); /* -- Sert uniquement pour demander une question -- */
        ...
        if (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTE ||
            ...
            if (bMono) { // Monospace
                if (bGras) {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontMonoBOLD,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontMonoBOLD, MaxFontSize);
                } else if (bItalic) {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontMonoITALIC,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontMonoITALIC, MaxFontSize);
                } else if (bGrasItalic) {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontMonoBOLDITALIC,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontMonoBOLDITALIC, MaxFontSize);

                } else {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontMono,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontMono, MaxFontSize);
                }
            } else { // Regular
                if (bGras) {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontRegBOLD,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontRegBOLD, MaxFontSize);

                } else if (bItalic) {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontRegITALIC,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontRegITALIC, MaxFontSize);

                } else if (bGrasItalic) {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontRegBOLDITALIC,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontRegBOLDITALIC, MaxFontSize);

                } else {
                    auto MaxFontSize = (float)GetMaxFontSize(pFontReg,
                                                             valLargeur,
                                                             valHauteur,
                                                             valTaillePolice,
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                             ...
                                                             (vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNEQUESTION ||
                                                              vecCommandeList[lArg].mTypeCommande == TypeCommande::DESSINETEXTEMULTILIGNE)
                                                               ? 1.1
                                                               : 1.0);
                    Painter.TextState.SetFont(pFontReg, MaxFontSize);
                }
            }
        }
        /* -------------- Preload des alignements -------------- */
        PoDoFo::PdfHorizontalAlignment mAlignLargeur = PoDoFo::PdfHorizontalAlignment::Left;
        PoDoFo::PdfVerticalAlignment mAlignHauteur = PoDoFo::PdfVerticalAlignment::Top;
        if (valAlignementLargeurI == 0)
            mAlignLargeur = PoDoFo::PdfHorizontalAlignment::Left;
        if (valAlignementLargeurI == 1)
            mAlignLargeur = PoDoFo::PdfHorizontalAlignment::Center;
        if (valAlignementLargeurI == 2)
            mAlignLargeur = PoDoFo::PdfHorizontalAlignment::Right;
        if (valAlignementHauteurI == 0)
            mAlignHauteur = PdfVerticalAlignment::Top;
        if (valAlignementHauteurI == 1)
            mAlignHauteur = PoDoFo::PdfVerticalAlignment::Center;
        if (valAlignementHauteurI == 2)
            mAlignHauteur = PoDoFo::PdfVerticalAlignment::Bottom;
        if (ValSplit > 1) {
            mAlignLargeur = PoDoFo::PdfHorizontalAlignment::Center;
            mAlignHauteur = PoDoFo::PdfVerticalAlignment::Center;
        }
        /* -------------- Preload du Painter -------------- */
        Painter.SetStrokeStyle(PdfStrokeStyle::Solid, false, valEpaisseur);
            ...
            case TypeCommande::DESSINERECTANGLEVIDE: {
                {
                    Painter.GraphicsState.SetStrokingColor(PdfColor((double)(ValRouge / 255.0),
                                                                    (double)(valVert / 255.0),
                                                                    (double)(valBleu / 255.0))); // Couleur ligne
                    Painter.DrawRectangle(PoDoFo::Rect(valDebutX, PageHeight - valDebutY - valHauteur, valLargeur, valHauteur));
                }
            } break;
            case TypeCommande::DESSINERECTANGLEGRILLE: {
                {
                    ...
                    for (qsizetype i = 1; i < ValGColonne; i++)
                        Painter.DrawRectangle(PoDoFo::Rect(valDebutX, PageHeight - valDebutY - valHauteur, i * GapColonne, valHauteur));
                    for (qsizetype i = 0; i < ValGLigne; i++) {
                        Painter.DrawRectangle(PoDoFo::Rect(valDebutX, PageHeight - valDebutY - valHauteur, valLargeur, i * GapLigne));
                    }
                }
            } break;
            case TypeCommande::DESSINERECTANGLEREMPLIS: {
                {
                    Painter.GraphicsState.SetStrokingColor(PdfColor((double)(ValRouge / 255.0),
                                                                    (double)(valVert / 255.0),
                                                                    (double)(valBleu / 255.0))); // Couleur ligne
                    Painter.GraphicsState.SetNonStrokingColor(PdfColor((double)(ValRemplisRouge / 255.0),
                                                                       (double)(valRemplisVert / 255.0),
            ...
            case TypeCommande::DESSINETEXTE: {
                {
                    Painter.GraphicsState.SetNonStrokingColor(PdfColor((double)(ValRouge / 255.0),
                                                                       ...
                                                                       (double)(valBleu / 255.0))); // Couleur ligne
                    if (ValSplit < 2) {
                        PoDoFo::PdfString mUtf8(QStringToPdfString(valTexte));
                        ...
                        for (qsizetype i = 0; i < ValSplit; i++) {
                            PoDoFo::PdfString mUtf8(QStringToPdfString(QString(valTexte.mid(i, 1))));
                            Painter.DrawTextMultiLine(mUtf8,
                ...
                QString valChemin = RetourneCleStr(vecCommandeList[lArg].mVecCommande, "chemin");
                {
                    try {
                        ...
                        if (QFile::exists(CheminImage)) {
                            ...
                            Painter.DrawImage(*image, valDebutX, PageHeight - valDebutY - valHauteur, mScaleW, mScaleH);
                        }
                    } catch (const PoDoFo::PdfError& e) {
                        EnvoiLogVersMainWindows(QString("INSEREIMAGE non effectué, erreur "
                                                        "de chargement de l'image '%1'")
                                                  .arg(valChemin));
                        EnvoiLogVersMainWindows(QString("Erreur interne : '%1'").arg(QString::fromLocal8Bit(e.what())));
            ...
            case TypeCommande::DESSINECHECKBOX: {
                {
                    ...
                    Painter.DrawRectangle(valDebutX, PageHeight - valDebutY - valHauteur, valLargeur, valHauteur);
                }
            } break;
            case TypeCommande::DESSINECHECKBOXQUESTION: {
                {
            ...
            case TypeCommande::DESSINEMULTICHECKBOXQUESTION: {
                {
                    Painter.GraphicsState.SetStrokingColor(PdfColor((double)(ValRouge / 255.0),
                                                                    (double)(valVert / 255.0),
                                                                    (double)(valBleu / 255.0))); // Couleur ligne
                    Painter.DrawRectangle(valDebutX1, PageHeight - valDebutY1 - valHauteur, valLargeur, valHauteur);
                    Painter.DrawRectangle(valDebutX2, PageHeight - valDebutY2 - valHauteur, valLargeur, valHauteur);
                    if (valDefautquestion == "oui") {
...
*/
double
PDGHelper::GetMaxFontSize(PoDoFo::PdfFont& Police,
                          double rectWidth,
                          double rectHeight,
...
{
    Q_UNUSED(CorrectionEspace);
    double taille = mTaillePolice;
    std::string texteUtf8 = Texte.toUtf8().toStdString();
    int nombreLignes = Texte.count('\n') + 1;
    ...
    while (taille > 0.0) {
        try {
            // Configurer l'état du texte
            painter.TextState.SetFont(Police, taille);

            // Mesurer le texte avec le painter
            double largeurTexte = painter.TextState.GetFont()->GetStringLength(texteUtf8, painter.TextState);

            // Calculer la hauteur de ligne
            const PoDoFo::PdfFontMetrics& metrics = Police.GetMetrics();
            double hauteurLigne;

            if (metrics.GetLineSpacing() > 0) {
                hauteurLigne = (metrics.GetLineSpacing() * taille) * nombreLignes;
            } else {
                hauteurLigne = ((metrics.GetAscent() - metrics.GetDescent()) * taille) * nombreLignes;
            }

            // Vérifier si le texte rentre
            if (largeurTexte <= rectWidth && hauteurLigne <= rectHeight) {
                break;
            }

        } catch (...) {
            // En cas d'erreur, passer à la taille suivante
        }
...
*/
QString
PDGHelper::RetourneCleStr(QVector<CmdKeys>& lVecKey, QString Cle, QString Defaut)
{
...
*/
double
PDGHelper::RetourneCleDouble(QVector<CmdKeys>& lVecKey, QString Cle)
{
    for (qsizetype i = 0; i < lVecKey.size(); i++) {
        if (lVecKey[i].Keys == Cle) {
...
*/
int
PDGHelper::RetourneCleInt(QVector<CmdKeys>& lVecKey, QString Cle)
{
    for (qsizetype i = 0; i < lVecKey.size(); i++) {
        if (lVecKey[i].Keys == Cle) {
...
*/
bool
PDGHelper::RetourneCleBool(QVector<CmdKeys>& lVecKey, QString Cle)
{
    for (qsizetype i = 0; i < lVecKey.size(); i++) {
        if (lVecKey[i].Keys == Cle) {
            return true;
...
void
PDGHelper::EnvoiLogVersMainWindows(QString Message)
{
    emit EnvoiLogMessage(Message);
}

PoDoFo::PdfString
PDGHelper::QStringToPdfString(const QString& qstr)
{
