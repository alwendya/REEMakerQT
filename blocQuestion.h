...
public:
    ...
    enum TypeBloc : int {Bloc_Texte_Simple = 0, Bloc_Texte_Multiligne = 2, Bloc_Case_Coche = 4};
    ...
    explicit blocQuestion(QWidget *parent = nullptr,ItemDefinition mIDEF = ItemDefinition(), bool Sombre = false);
...
private:
    ...
    public:
        MyValidatorLOWER(QObject* parent=nullptr): QValidator(parent) {}
        State validate(QString& input, int&) const override {
            input = input.toUpper();
            return QValidator::Acceptable;
        }
    ...
    public:
        MyValidatorDECIMAL(QObject* parent=nullptr): QValidator(parent) {}
        State validate(QString& input, int&) const override {
            QString result;
    ...
    protected:
        bool eventFilter(QObject *obj, QEvent *event) override
        {
            if (event->type() == QEvent::KeyPress || event->type() == QEvent::KeyRelease) {
                ...
                if (
                        (keyEvent->key()==Qt::Key_Shift) || (keyEvent->key()==Qt::Key_Backspace)
                        || (keyEvent->key()==Qt::Key_Alt)|| (keyEvent->key()==Qt::Key_Delete)
